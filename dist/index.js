...
const resolvers = {
    Query: {
        repositories: async (_, { username, token }) => {
            try {
            ...
            `,
                ...
                return response.data.data.user.repositories.edges.map(({ node }) => node);
            }
            catch (error) {
        ...
        repositoryDetails: async (_, { username, token, repoName }) => {
            try {
                const repositoryResponse = await axios.post('https://api.github.com/graphql', {
                    query: `
              query {
            ...
            `,
                ...
                const repoDetailsResponse = await axios.get(`https://api.github.com/repos/${username}/${repoName}`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                }).catch(err => {
                ...
                const yamlFileContentResponse = await axios.get(`https://api.github.com/repos/${username}/${repoName}/contents/*.yml`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                }).catch(err => {
                ...
                const webhooksResponse = await axios.get(`https://api.github.com/repos/${username}/${repoName}/hooks`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                }).catch(err => {
